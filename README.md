...
Rev App - это Flutter приложение для мониторинга активности во фракциях игры Revelation Online. Приложение позволяет отслеживать прогресс по каждой фракции и рассчитывать время до достижения цели. Также включает функционал "Клуб знатоков" для поиска ответов на вопросы из ивента игры.
...
- ✅ Обработка ошибок при запуске приложения (показ информативного экрана вместо черного экрана)
- ✅ Экспорт и импорт базы данных (резервное копирование данных через прямое копирование файла БД)
- ✅ Клуб знатоков - поиск ответов на вопросы из ивента игры (поиск по тексту вопроса и ответа)
...
- `share_plus` - экспорт файлов (поделиться файлом)
