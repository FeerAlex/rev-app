...
- Содержит три раздела:
  ...
  3. **Клуб знатоков** - открывает страницу поиска ответов на вопросы
...
Функционал "Клуб знатоков" предназначен для поиска ответов на вопросы из ивента "Клуб знатоков" в игре Revelation Online. Поддерживает как текстовый поиск, так и распознавание вопросов с фотографий через OCR.

### Как использовать

**Текстовый поиск:**
1. Откройте меню навигации (проведите пальцем от левого края экрана или нажмите на иконку меню)
2. Нажмите на пункт **"Клуб знатоков"**
3. На странице поиска введите текст вопроса или ответа в поисковую строку
4. Результаты поиска отобразятся в виде карточек с вопросами и ответами

**Распознавание вопроса с фотографии (OCR):**
1. Откройте страницу "Клуб знатоков"
2. Нажмите на иконку камеры в поисковой строке
3. Наведите камеру на вопрос так, чтобы он попадал в рамку на экране
4. Нажмите кнопку съемки (белая круглая кнопка внизу)
5. Приложение автоматически распознает текст с изображения и найдет похожий вопрос в базе
6. Если вопрос найден, он автоматически заполнится в поисковую строку
7. Если вопрос не найден, отобразится распознанный текст для ручного поиска

### Поиск

**Текстовый поиск:**
- Поиск выполняется одновременно по тексту вопроса и всем ответам из массива `answers`
- Поиск не учитывает регистр (заглавные и строчные буквы)
- Поиск находит частичные совпадения (если введете часть слова, найдутся все вопросы/ответы, содержащие эту часть)
- При пустом поисковом запросе отображаются все доступные вопросы
- Поиск выполняется мгновенно по мере ввода текста

**OCR распознавание:**
- Использует Tesseract OCR для распознавания текста с фотографий
- Поддерживает русский язык
- Автоматически обрабатывает изображение для улучшения качества распознавания:
  - Конвертация в grayscale
  - Удаление шума
  - Увеличение контрастности
  - Бинаризация изображения
  - Масштабирование для маленьких изображений
- Использует fuzzy search (нечеткий поиск) для поиска похожих вопросов по распознанному тексту
- Алгоритм Levenshtein distance для вычисления сходства текста
- Порог совпадения: 60% (настраивается)
- Если вопрос найден с коэффициентом совпадения выше порога, он автоматически заполняется в поисковую строку

### Структура данных

Вопросы и ответы хранятся в файлах:
- `assets/questions/questions_club.json` - вопросы "Клуба знатоков"
- `assets/questions/questions_theosophy.json` - вопросы теософии
...
**Примечание:** Поле `answers` - это массив строк. Вопрос может иметь один или несколько правильных ответов. Все ответы отображаются в карточке в виде отдельных чипсов.

### Советы по использованию OCR

1. **Качество съемки:**
   - Убедитесь, что вопрос четко виден в рамке
   - Избегайте бликов и теней
   - Держите устройство стабильно

2. **Обработка результатов:**
   - Если вопрос не найден автоматически, используйте распознанный текст для ручного поиска
   - Проверьте распознанный текст на наличие ошибок OCR
   - При необходимости отредактируйте текст в поисковой строке

3. **Точность распознавания:**
   - Качество распознавания зависит от качества изображения
   - Приложение автоматически обрабатывает изображение для улучшения результатов
   - Fuzzy search помогает найти вопросы даже при небольших ошибках распознавания

### Добавление вопросов

Для добавления новых вопросов:
1. Откройте файл `assets/questions/questions_club.json` в корне проекта
2. Добавьте новый объект с полями `id`, `question` и `answer`
3. Сохраните файл
4. Перезапустите приложение или обновите список (потяните список вниз)

**Примечание:** ID должен быть уникальным для каждого вопроса. Рекомендуется использовать последовательные числа, начиная с 1.

### Обновление данных

Список вопросов можно обновить вручную:
1. Потяните список вниз (свайп вниз)
2. Данные перезагрузятся из JSON файла
...
Приложение поддерживает экспорт и импорт базы данных для создания резервных копий и восстановления данных.

### Экспорт базы данных

Для создания резервной копии всех данных:

1. Откройте меню навигации (проведите пальцем от левого края экрана или нажмите на иконку меню)
2. Нажмите на пункт **"Экспорт БД"**
3. Выберите способ экспорта:
   - Сохранить в файловый менеджер
   - Отправить по email
   - Сохранить в облачное хранилище (Google Drive, Dropbox и т.д.)
   - Другие доступные варианты на вашем устройстве
4. Файл `rev_app.db` будет экспортирован

**Что сохраняется:**
- Все данные о фракциях (включая скрытые)
- Текущая валюта и опыт
- Настройки активностей (заказы, работы)
- Прогресс по украшениям
- Цели (целевой уровень репутации, нужность сертификата)
- Порядок отображения фракций
...
3. В открывшемся диалоге подтвердите импорт
...
6. После успешного импорта данные автоматически перезагрузятся

**Важно:**
- Импорт полностью заменяет текущую базу данных
- Все текущие данные будут потеряны после импорта
- Импортируемый файл должен быть валидной SQLite базой данных с таблицей `factions`
- При ошибке импорта текущие данные останутся без изменений

### Рекомендации

- **Регулярно делайте резервные копии** - особенно перед обновлением приложения или при значительных изменениях данных
- **Храните копии в безопасном месте** - используйте облачное хранилище или внешний носитель
- **Проверяйте файлы перед импортом** - убедитесь, что файл был экспортирован из этого приложения
- **Делайте копию перед импортом** - если вы не уверены в содержимом импортируемого файла, сначала экспортируйте текущие данные
