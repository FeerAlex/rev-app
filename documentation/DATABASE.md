...
**Примечание:** 
- Настройки приложения (цены, валюты) хранятся как константы в коде (`lib/data/datasources/app_settings.dart`), а не в базе данных
...
База данных хранится локально на устройстве. Приложение поддерживает экспорт и импорт базы данных через прямое копирование файла БД.

### Экспорт базы данных

Экспорт позволяет создать резервную копию всех данных приложения:

1. Откройте меню навигации (Drawer)
2. Нажмите на пункт **"Экспорт БД"**
3. Выберите способ экспорта (сохранить в файловый менеджер, отправить по email и т.д.)
4. Файл `rev_app.db` будет экспортирован

**Примечание:** Экспортированный файл содержит все данные о фракциях, включая скрытые фракции, настройки, прогресс и цели.

### Импорт базы данных

Импорт позволяет восстановить данные из ранее экспортированного файла:

1. Откройте меню навигации (Drawer)
2. Нажмите на пункт **"Импорт БД"**
3. Подтвердите импорт в диалоге (внимание: импорт заменит все текущие данные)
4. Выберите файл `rev_app.db` для импорта
5. Приложение проверит валидность файла и заменит текущую БД

**Важно:**
- Импорт полностью заменяет текущую базу данных
- Рекомендуется сделать резервную копию перед импортом
- Импортируемый файл должен быть валидной SQLite базой данных с таблицей `factions`
- После успешного импорта данные автоматически перезагружаются в приложении

### Альтернативные способы резервного копирования

Для ручного резервного копирования можно скопировать файл базы данных напрямую с устройства:
- **Android:** `/data/data/com.example.rev_app/databases/rev_app.db` (требуются root права)
- **iOS:** `Documents/rev_app.db` (через iTunes File Sharing)
