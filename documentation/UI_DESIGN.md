...
- **Блок инвентаря** (`FactionInventoryBlock`) - валюта (редактируемое), текущий уровень репутации (dropdown), опыт на уровне (редактируемое), сертификат (чекбокс "Есть сертификат")
