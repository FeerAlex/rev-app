...
- `CertificateIcon` - иконка сертификата справа от валюты (кликабельно для переключения статуса)
- `FactionActivitiesList` - список активностей с бейджами (Заказ, Доска)
- `ActivityBadge` - отдельный бейдж активности (переиспользуемый)
- `BoardCurrencyBadge` - бейдж валюты с доски (кликабельно для редактирования)
...
**Примечание:** Валюту и валюту с доски можно редактировать прямо из карточки фракции, поэтому они не отображаются на странице редактирования.
...
**Bottom Navigation Bar (на странице фракций):**
- Темный фон (`#0F0F0F`)
- Оранжевый цвет для выбранного элемента
- Серый цвет для невыбранных элементов
- Две вкладки: Список, Настройки
