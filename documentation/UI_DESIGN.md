...
```
┌─────────────────────────────────────┐
│ Название                    Валюта: 1000 │
│ [Заказ] [Работа]        Время: 2 д 5 ч │
│ ───────────────────────────────────── │
│ [Progress Bar: 5000/43000]  2 д 3 ч │
└─────────────────────────────────────┘
```
...
- `TimeToGoalWidget` - компактное отображение времени до цели по валюте в формате "X д Y ч Z м"
- `TimeToReputationGoalWidget` - компактное отображение времени до цели по репутации в формате "X д Y ч Z м" (без иконок)

### Страница деталей фракции

Страница редактирования фракции оптимизирована и включает:

- **Блок ежедневных активностей** (`FactionActivitiesBlock`) - галочки "Заказы" и "Работы" в одной строке
- **Блок сертификата** (`FactionCertificateBlock`) - отдельный блок с галочкой "Сертификат"
...
- **Скрытие фракции** - кнопка скрытия в AppBar (иконка удаления)

**Примечание:** 
- Валюту и валюту с работы можно редактировать прямо из карточки фракции, поэтому они не отображаются на странице редактирования
- Галочки отображаются только для тех активностей, которые доступны для фракции согласно статическому списку (`FactionTemplate`)
...
- **Виджет времени по валюте** (`TimeToGoalWidget`) - показывает предыдущее значение при пересчете, индикатор загрузки только при первой загрузке, отображается справа вверху карточки
- **Виджет времени по репутации** (`TimeToReputationGoalWidget`) - показывает предыдущее значение при пересчете, индикатор загрузки только при первой загрузке, отображается справа от progress bar опыта внизу карточки
